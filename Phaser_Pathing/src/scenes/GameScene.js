...
export default class GameScene extends Phaser.Scene {
    ...
    async create() {
        ...
        // ============================================================
        // LOGOUT BUTTON (Top-right with 80px padding)
        // ============================================================
        ...
        this.tweens.add({
            ...
            ease: 'Sine.easeInOut'
                ...
                // ============================================================
        // LOAD SAVE DATA (difficulty, settings, menu state, etc.)
        // ============================================================
        const saveData = saveManager.getData ? saveManager.getData() : null;
        this.settingsData = (saveData && saveData.settings) ? { ...saveData.settings } : {
            masterVolume: 0.8,
            musicVolume: 0.6,
            sfxVolume: 0.7,
        ...
        let selectedDiff = (saveData && saveData.selectedDifficulty) ? saveData.selectedDifficulty : 'NORMAL';
        const lastMenuPage = saveData && saveData.lastMenuPage ? saveData.lastMenuPage : null;
        ...
        const createPage = () => {
            ...
            back.on('pointerdown', () => {
                this.hideAllPages();
                this.showMenu();
                saveManager.save({ lastMenuPage: null });
            });
        ...
        this.playPage.pageKey = 'play';
        this.towersPage = createPage();
        this.towersPage.pageKey = 'towers';
        this.upgradesPage = createPage();
        this.upgradesPage.pageKey = 'upgrades';
        this.settingsPage = createPage();
        this.settingsPage.pageKey = 'settings';
        ...
        const showPage = (page) => {
            ...
            if (page.pageKey) {
                saveManager.save({ lastMenuPage: page.pageKey });
            }
        };
        ...
        const diffButtons = difficulties.map((d, i) => {
            ...
            t.on('pointerdown', () => {
                ...
                t.setColor('#ff2a2a');
                saveManager.save({ selectedDifficulty: selectedDiff });
            });
        ...
        this.upgradesPage.add(upgradeTooltip);
                ...
                // ============================================================
        // SETTINGS PAGE (with saveManager-backed sliders & toggles)
        // ============================================================
        ...
        const createSlider = (label, y, initialValue, settingKey) => {
            ...
            const barWidth = this.scale.width * 0.30;
            const barCenterX = this.scale.width * 0.60;
            ...
            const knobX = this.scale.width * 0.45 + initialValue * barWidth;
            ...
            const updateFromPointer = (pointerX) => {
                const left = barCenterX - barWidth / 2;
                let t = (pointerX - left) / barWidth;
                t = Phaser.Math.Clamp(t, 0, 1);

                const newX = this.scale.width * 0.45 + t * barWidth;
                knob.x = newX;

                this.settingsData[settingKey] = t;
                saveManager.save({
                    settings: {
                        ...this.settingsData
                    }
        ...
        const masterSlider = createSlider(
            'Master Volume',
            this.scale.height * 0.40,
            this.settingsData.masterVolume ?? 0.8,
            'masterVolume'
        );
        const musicSlider = createSlider(
            'Music Volume',
            this.scale.height * 0.50,
            this.settingsData.musicVolume ?? 0.6,
            'musicVolume'
        );
        const sfxSlider = createSlider(
            'SFX Volume',
            this.scale.height * 0.60,
            this.settingsData.sfxVolume ?? 0.7,
            'sfxVolume'
        );
        const sensSlider = createSlider(
            'Mouse Sensitivity',
            this.scale.height * 0.70,
            this.settingsData.mouseSensitivity ?? 0.5,
            'mouseSensitivity'
        );

        const createToggle = (label, y, initialOn, settingKey) => {
            const labelText = this.add.text(
                this.scale.width * 0.30,
                y,
                label,
            ...
            const toggle = () => {
                ...
                const newOn = !on;
                this.settingsData[settingKey] = newOn;
                saveManager.save({
                    settings: {
                        ...this.settingsData
                    }
        ...
        const gfxToggle = createToggle(
            'Graphics: High',
            this.scale.height * 0.80,
            this.settingsData.graphicsHigh ?? true,
            'graphicsHigh'
        );
        const fullscreenToggle = createToggle(
            'Fullscreen',
            this.scale.height * 0.88,
            this.settingsData.fullscreen ?? false,
            'fullscreen'
        );
        ...
        // ============================================================
        // INITIAL MENU STATE (restore last page if any)
        // ============================================================
        if (lastMenuPage && this[`${lastMenuPage}Page`]) {
            this.hideMenu();
            showPage(this[`${lastMenuPage}Page`]);
        } else {
            this.showMenu();
        }
